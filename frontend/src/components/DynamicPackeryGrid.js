...
const DynamicPackeryGrid = ({ data }) => {
  ...
  const generateSampleBoxes = async () => {
    ...
    await setTimeout(() => {
      ...
      const containerWidth = 530; // Fixed width
  ...
  useEffect(() => {
    generateSampleBoxes();
    setTimeout(async () => {
      // await addPuzzleNotches();
      // setNotches(false);
    }, 1000);
  }, [data]);
  // notch swich. TUrn on and off to see the notches
  const [notches, setNotches] = useState(false);
  const toggleNotches = () => {
    setNotches(!notches);
  };
    ...
    <div className="w-[900px] mx-auto">
      <div className="control-panel">
        <div className="absolute-button-container">
          ...
          <button onClick={toggleNotches} className="absolute-button">
            {notches ? "Hide Notches" : "Show Notches"}
          </button>
        </div>
      </div>

      <div className="masked-container flex justify-center w-[530px] ml-1">
